...
using (IDbConnection conn = connection.SqlConnection)
{
    conn.Open();
    ...
    do
    {
        ...
        if (choice == 1)
        {
            ...
            try
            {
                ...
                MyLogger.Log($"Try to insert", LogLevel.Information);
                Product product = new Product(name, category, provider, price);
                long rows = conn.Insert<Product>(product);

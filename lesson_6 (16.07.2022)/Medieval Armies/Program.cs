...
using Medieval_Armies.Exceptions;
...
for (short i = 0; i < first_group_soldiers + second_group_soldiers; i++)
{
    ...
    if (choice_of_origin < 1 || choice_of_origin > 4)
    {
        try
        {
            throw new ChoiceOfOrigionException();
        }
        catch (Exception ex)
        {
    ...
    Console.WriteLine($"If you want {iterator}-{suffix} warrior to be on a camel - enter '1';");
    Console.WriteLine($"If you want {iterator}-{suffix} warrior to be on a horse - enter '2';");
    Console.Write($"If you want {iterator}-{suffix} warrior to be on foot - enter '3': ");
    short choice_of_moving = short.Parse(Console.ReadLine());
    if (choice_of_moving < 1 || choice_of_origin > 3)
    {
        try
        {
            throw new ChoiceOfMovingException();
        }
        catch (Exception ex)
        {
            Console.WriteLine(ex.Message);
